...
export default function DetailQrFormatPage() {
  const router = useRouter();
  const params = useParams();
  const encryptedId = params?.id;

  const [qrFormat, setQrFormat] = useState(null);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const response = await fetchData(`qr-formats/${encryptedId}/detail`, {}, "GET");
      ...
      setQrFormat(detail.qrFormat || null);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetchData(`qr-formats/${encryptedId}/assign-customers`, { customerIds: selectedCustomers }, "POST");
      ...
      const logResponse = await createActionLog({
        action: "UPDATE",
        oldValue: null,
        newValue: `QR Format: ${qrFormat?.Name || "-"}, Customer assigned: ${selectedCustomers.join(", ") || "-"}`,
      });
  ...
  return (
    <>
      ...
      <Breadcrumb
        title="Detail QR Format"
        items={[
          { label: "QR Formats Management", href: "/pages/qr-format" },
          { label: "Detail" },
        ]}
      />
      <div className="card border-0 shadow-sm mb-3">
        <div className="card-body p-4">
          <div className="row">
            <InfoItem label="Format Name" value={qrFormat?.Name} />
            <InfoItem label="Seq Length" value={qrFormat?.SeqLength} />
            <InfoItem label="Status" value={qrFormat?.Status === 1 ? "Active" : "Inactive"} />
            <div className="col-md-3 col-sm-12 mb-3 d-flex justify-content-end align-items-start">
          ...
          <div className="d-flex justify-content-between align-items-center p-3 border-bottom gap-2">
            <div>
              ...
              <p className="text-secondary mb-0" style={{ fontSize: 13 }}>
                Select customers for this QR format.
              </p>
            </div>
