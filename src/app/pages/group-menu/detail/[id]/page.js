...
export default function DetailGroupMenuPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      const response = await fetchData(`group-menus/${encryptedId}/detail`, {}, "GET");
      ...
      const groupData = detail.groupMenu || null;
      const assigned = Array.isArray(detail.assignedMenuIds) ? detail.assignedMenuIds : [];

      setGroupMenu(groupData);
      setSelectedMenus(assigned);

      const groupedMenus = [...(detail.menus || [])]
        .filter((item) => /^\/pages\/[^/]+$/.test(String(item.Path || "").trim()))
        .filter((item) => {
          const menuGroupId = item.GroupId == null ? null : Number(item.GroupId);
          const currentGroupId = groupData?.Id == null ? null : Number(groupData.Id);
          return menuGroupId === null || menuGroupId === currentGroupId;
        })
        .sort((a, b) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetchData(`group-menus/${encryptedId}/assign-menus`, { menuIds: selectedMenus }, "POST");
      if (response.error) throw new Error(response.message);

      const logResponse = await createActionLog({
        action: "UPDATE",
        oldValue: null,
        newValue: `Group Menu: ${groupMenu?.Name || "-"}, Menu assigned: ${selectedMenus.join(", ") || "-"}`,
      });

      if (logResponse.error) {
        throw new Error(logResponse.message || "Group menu saved, but action log failed");
      }

      Toast.success(response.message || "Group menu data saved successfully");
      router.push("/pages/group-menu");
    } catch (error) {
      Toast.error(error.message || "Failed to save group menu data");
    } finally {
      setSaving(false);
    }
  };
    ...
    <>
      ...
      <div className="card border-0 shadow-sm mb-3">
        <div className="card-body p-4">
          <div className="row">
            <InfoItem label="Group Menu Name" value={groupMenu?.Name} />
            <InfoItem label="Status" value={groupMenu?.Status === 1 ? "Active" : "Inactive"} />
